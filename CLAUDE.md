...
> **Current build: v194** (Playwright ~720 tests — full-suite runs green with rare singleton parallel-load flakes that pass 3/3 isolated · Firestore rules emulator 108/108 · backend suites 94/94 · §4 benchmark green). Recent (v183→v194, one founder live-test sprint): v183 light toggle, v184 AI-honesty, v185 SEO canonical, v186 seen-ledger [WRITE] + D1 diagnosis, v187 education/certs matching, v188 light contrast, v189 toast fix, v190 intuitive keyword search, v191 Improve-✕ cancels, v192 checked-skill survives the cap, v193 signup dark/light picker, v194 Enter-to-search-whole-region + nationwide expand. **D1 — CAUSE FOUND + FIXED (2026-08-09): the pool builder OOM-crashed the runner ("JavaScript heap out of memory") — an unbounded `.get()` of all active jobs. Fixed: `scripts/build_job_pool.mjs` now STREAMS + trims each doc on arrival (~5× less peak memory) via `poolsFromRows`, + the workflow runs it with `--max-old-space-size=6144`. Founder just needs to re-run "Daily Job Harvest"; the v186 Verify step confirms `job_pools/_manifest` writes. Then deck reads ~3,800→~6 and the reverse-match (#16, same secret) likely lights up. See `memory/gpj-d1-pool-never-written.md`.** **Founder-queued next (live tracker): public résumé checker ([UI-REVIEW], built on `api/rate/rateCore.js` — deliberately STRUCTURAL/zero-AI so it's free-at-scale; AI is the signed-in payoff); Inbox tab + per-message dismiss both sides ([UI-REVIEW] — reshapes the 6-tab reskin, needs care re: the candidate-first invariant); Match-to-Job before/after audit.**
