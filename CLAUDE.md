...
> **Current build: v187** (Playwright ~710 tests — full-suite runs green with rare singleton parallel-load flakes that pass 3/3 isolated · Firestore rules emulator 108/108 · backend suites 94/94 · §4 benchmark green). Recent: v183 light-mode toggle, v184 AI-honesty, v185 SEO canonical, v186 compact seen-ledger [WRITE] + D1 diagnosis, v187 match reads education/certs. **Known open (founder-gated): the D1 job pool has NEVER been written — `job_pools/_manifest` is 404 live; the FIREBASE_SERVICE_ACCOUNT secret is VALID (the Python harvester writes jobs with it), so the failure is in the Node pool-build step — needs the CI step log to pin. See `memory/gpj-d1-pool-never-written.md`.**
