...
> **Current build: v192** (Playwright ~716 tests — full-suite runs green (715 passed / 1 skipped) with rare singleton parallel-load flakes that pass 3/3 isolated · Firestore rules emulator 108/108 · backend suites 94/94 · §4 benchmark green). Recent (v183→v192, one founder live-test sprint): v183 light-mode toggle, v184 AI-honesty, v185 SEO canonical, v186 seen-ledger [WRITE] + D1 diagnosis, v187 match reads education/certs, v188 light-mode contrast softened, v189 toast `&amp;` fix, v190 intuitive keyword search, v191 Improve-✕ cancels, v192 checked-skill survives the 15-cap. **Known open (founder-gated): the D1 job pool has NEVER been written — `job_pools/_manifest` is 404 live; the FIREBASE_SERVICE_ACCOUNT secret is VALID (the Python harvester writes jobs with it), so the failure is in the Node pool-build step — needs the CI step log to pin. See `memory/gpj-d1-pool-never-written.md`.** **Founder-queued next (see the live tracker): progressive local→nationwide keyword widening; public résumé checker ([UI-REVIEW], built on `api/rate/rateCore.js`); Inbox tab + per-message dismiss both sides ([UI-REVIEW]); dark/light choice during signup ([UI-REVIEW]); Match-to-Job before/after audit.**
