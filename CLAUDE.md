...
> **Current build: v196** (Playwright ~724 tests — full-suite runs green with rare singleton parallel-load flakes that pass 3/3 isolated · Firestore rules emulator 108/108 · backend suites 94/94 · §4 benchmark green). Recent (v183→v196, one founder live-test sprint): light mode (v183/v188/v193 signup picker), AI-honesty (v184), SEO canonical (v185), seen-ledger [WRITE] + D1 diagnosis (v186), education/certs matching (v187), toast fix (v189), intuitive keyword search (v190), Improve-✕ cancels (v191), checked-skill survives the cap (v192), Enter-to-search-whole-region (v194), footer link to the checker (v195), **per-message dismiss + collapse (v196)**. **Also shipped (no app version): the public `/resume-checker.html`** — free, zero-AI, zero-read Résumé Strength Checker (upload PDF/DOCX/TXT or paste/LinkedIn), rateCore inlined single-source via `scripts/build_resume_checker.mjs`, static (0 functions), in the sitemap + homepage footer. **D1 — CAUSE FOUND + FIXED: the pool builder OOM-crashed ("JavaScript heap out of memory"); `scripts/build_job_pool.mjs` now STREAMS + trims each doc (`poolsFromRows`) + `--max-old-space-size=6144`. Founder re-runs "Daily Job Harvest"; the v186 Verify step confirms `job_pools/_manifest`. See `memory/gpj-d1-pool-never-written.md`.** **Founder-queued next: full Inbox TAB (relocate messages out of Settings, both sides — a 13-touch-point nav/reskin change, needs a focused pass to protect the candidate-first invariant; v196 shipped the safe dismiss+collapse interim); the Resources/blog SEO engine (daily GitHub Action → data-driven articles from the jobs DB, AI-assisted but DATA-GROUNDED never fabricating stats, byline "The GhostProofJob Team" for market data / "Jett" for résumé tips).**
...
10. **[FREE-TIER] $0 infrastructure budget.** GPJ runs on FREE plans by design — the founder pays for the AI resume helper and Claude, nothing else. **Before adding any endpoint, workflow, cron, storage bucket, or third-party service, state the platform quota it consumes and confirm the headroom.** A build that is perfect in code still fails if it exceeds a quota — that is a planning failure. Prefer designs that consume no per-user quota (static/CDN output, pre-computed snapshots, client-side compute) over per-request backend work. **Known ceilings:** Vercel Hobby = **12 Serverless Functions per deployment** (every `api/` .js file counts, *including helper modules that are not handlers* — this has hard-failed deploys **twice**; keep non-handlers in `.vercelignore`); Firestore free tier = 50K reads/day (Blaze trial credit covers overage only until 2026-09-19); sitemap = 50,000 URLs per file. Benchmark step **[8] free-tier platform limits** enforces these — it counts deployed functions, catches orphaned `vercel.json` `functions` patterns, verifies deployed relative `require`s resolve with exact case (Vercel's FS is case-sensitive, Windows/macOS are not), and checks the sitemap cap.
...
- **v123–v182** — extended founder live-test iteration (résumé/rater/cover-letter/apply engine + recruiter tier). Highlights: v161 the core PDF write-back fix (tailored bullets finally reach the PDF); v171–v172 Match-Preferences demo-data self-heal + Role-Fit honesty (caps at 35 out-of-field, no cross-field inflation); v174 résumé-corruption P0 ([object Object] coercion at every write-back); v177 Jett updates the master so the score moves; v178 hard metric-preservation guard; v179 "Résumé Strength" rubric; v180 removed the blended Overall number; v181 outcome-elicitation (asks for measurable results); v182 put-back removal tombstone (un-apply/un-skip survive the cloud merge).
- **v183–v187** — v183 **light-mode toggle** (opt-in; dark stays default; `:root[data-theme="light"]` token swap + additive overrides, dark byte-untouched; pre-paint no-flash script; toggle in the profile menu). v184 AI-honesty (stale "No resume yet?" CTA hides once a résumé exists; cover-letter WHOLE-WORD semantic match; refuse-and-tell for thin postings). v185 **SEO** homepage self-canonical + OG/Twitter (fixes GSC "duplicate without canonical"; the 312 "discovered not indexed" pages are templated-content crawl reality, NOT a bug — job/company-page indexing stays PARKED). v186 **compact "Hide Ledger" [WRITE]** — a keys-only 5,000-cap cloud-synced de-recycle memory (`gpj_seen_keys` → profile `seenKeys`) so jobs stop recycling past the 60-row list cap; rollback `gpjClearSeenLedger()`. v186 also DIAGNOSED D1 (pool never written) + added a workflow verify step. v187 **#17 match reads education + certs** — `_gpjScoreMatch` gets bounded positive-only credential signals (named-cert boost counts as relevance; degree = tiny nudge that can't bypass the cross-field cap; never a penalty); `computeMatch` passes edu/eduStruct/eduExtra + certs.
- **v188–v192** — one continuous founder live-test sprint (all fixes runtime-verified + full-suite green before deploy). v188 **light-mode contrast softened** (founder: "aggressive on the eyes" — off-white panels `#FBFAFE` + softer ink `#332E45` ≈ AAA, only the `[data-theme="light"]` block; + Quick Start theme-switch line). v189 fixed a literal `&amp;` in the un-save-company toast (showToast sets textContent). v190 **intuitive keyword search** — one shared `_gpjKeywordMatch(kw,title,company)` across all three Browse paths matches the TITLE only (single word = substring; multi-word = all words in title), never the description (searching "Account" no longer returns a "Project Coordinator" that merely mentioned accounts); + the deck's "N loaded — start swiping" toast is suppressed when a keyword is active (it counted the résumé-anchored deck pool, which ignores the keyword). v191 **the ✕ on the "Improve My Whole Resume" outcome prompt now CANCELS** (confirm-to-close) instead of `skipOutcomeElicit()` which still ran the improve — an accidental ✕ was overwriting the master résumé. v192 **a checked Match-to-Job "I have X" skill now leads the skills line** (`unshift`, not `push`) so it survives `_tidySkills`' first-15 cap — on a 15-skill résumé the appended #16 (e.g. "Campaigns") was silently trimmed before render; root cause found + runtime-proven, not guessed.
- **v193–v194 + D1 OOM fix** — v193 **dark/light choice during signup** (compact "Appearance" segmented control on the auth modal, applies live + persists via the same `gpj_theme`/`gpjApplyTheme`; `gpjSetThemeFromAuth`; note: avoid `on*`-prefixed local var names — `onS`/`offS` collided with the benchmark's `on*=` handler-audit regex). v194 **progressive keyword search** — pressing **Enter** in `f-keyword` runs `searchAllJobsForKeyword(false)` (the whole REGIONAL DB, local+remote, not just the loaded batch), and every keyword result offers "🌎 Also search nationwide →" (whole DB), hidden once `window._kwWasNationwide`; the full-region fetch is Enter-gated (not per-keystroke) for [FREE-TIER]. **D1 OOM fix** (script + workflow, no app version bump): `build_job_pool.mjs` streams + trims (`poolsFromRows` split out; `buildPool` kept as the unit-tested wrapper) + `--max-old-space-size=6144` — fixes the "heap out of memory" that had kept the pool unbuilt.
