...
> **Current build: v101** (Playwright 146/146 — the 130 v100 benchmark held + 8 new v101 tests; NEW browser-in-the-loop rule: every visual/flow fix is reproduced AND re-verified in a real browser with before/after screenshots — see `docs/v101-screens/`). v100 = smart-data finish: the DECK's inline mapper (not `mapFirestoreJob`!) still sliced desc at 460 raw and carried NO req/benefits/summary — now mirrors the v97 ceilings (11000/4000/2500) and carries all four fields; both `loadCompanyJobs` cmJobsCache paths + Lever/Ashby ATS rows raised to the same ceilings. Recruiter tier R1 (frontend) shipped in v99 — see `docs/recruiter-tier.md` + `docs/R0-report.md`. Recruiter jobs/apply (R2) not built yet.
