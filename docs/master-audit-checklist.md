...
### B-TEXT-CLIP · Site-wide "no incomplete text" audit  ·  ETA v84  ·  founder-directed
**What it is:** text anywhere (job cards, company cards, match cards, popups) must never render incomplete or visually cut. v83 fixed the reported match-insight title (24-char slice); a systematic sweep of every `.slice(`/`text-overflow`/fixed-height render remains — make popups size dynamically or wrap, optimized visually. Candidates found in passing: deck badge `s-loc` 22-char slice (kept — badge, full loc in header), stat rows, saved-job rows. Audit + fix per surface next batch.
