...
**Current build:** v75 (built 2026-07-03 from live v74 feedback). v75 = B-APPLY-CONFIRM (confirm-only apply, founder-directed) + B-DEMO-FLAG (sign-in gate + placeholder-report scrub) + B-DESC-CUT (word-boundary + full drawer text) + B-OPENCARD (company-card job panel populates + reveals). v74 = B-DECK-POOL + B-THISROLE (LIVE-CONFIRMED: self-test 19/0, 3000 jobs read).
**Updated:** 2026-07-03
...
### B-BENEFITS-VERIFY · Benefits "not written to Firebase" — pipeline verified in code; LIVE Firestore doc check needed  ·  ETA v76
**Finding (2026-07-03):** the whole benefits pipeline is present in the repo — harvester `extract_benefits` → `normalize_row` includes `benefits` → `batch.set` writes the full record; `firestoreWriter.js`/`regionalRouter.js` carry it on the ATS path; `mapFirestoreJob` maps it; the drawer + company job panel render it when non-empty. Two live explanations to check: (1) the deployed harvest may predate the benefits-capable harvester — verify the `job_harvest.yml` Actions run date and inspect a fresh `jobs` doc for a `benefits` field; (2) many postings genuinely have no benefits section (extraction correctly returns ''). Founder: open a fresh expanded card after the next harvest and/or check one Firestore doc.
