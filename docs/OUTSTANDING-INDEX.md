...
> **🚨 FREE-TIER CRISIS + PLAN A (2026-09-22):** the Blaze trial credit expired **2026-09-19**, dropping the project to Spark (free: 50K reads/day, 20K writes/day). Live Firestore began returning **HTTP 429 `RESOURCE_EXHAUSTED`**, which took down the harvest, reverse-match, resources-article workflows AND the live app (keyword search / deck returned nothing). **Root cause:** the D1 pool build streamed **all ~205K active jobs** back out of Firestore daily = **~4× the 50K read cap**; that drained the shared daily quota so everything else 429'd as collateral. **PLAN A (fix, this session — backend only, no `index.html` change):** the harvester now dumps its already-scraped rows to a JSONL file (`POOL_ROWS_FILE`), and `build_job_pool.mjs` builds the pool **from that file + the prior pool (~45 reads for metro accumulation)** instead of the 205K stream. Reads/day for the build: **~205K → ~45**. reverse-match (≤500 internal jobs) + resources (1 `_market_stats` read) were only collateral and get their quota back automatically. Client, `getJobFull`, writes = unchanged. New safety floor: never overwrites a good pool with < 200 rows (a scrape outage can't blank the deck). Tests: `tests/growth/poolBuild.test.mjs` (5 — dedup/stale/**metro accumulation**/trim+matchTerms/null-safe). **RECOVERY IS GATED ON:** (1) this fix deployed (done), (2) the harvest running with it (founder: trigger the **Daily Job Harvest** workflow, ideally after the daily read quota resets ~midnight Pacific / 08:00 UTC), (3) the read quota resetting. After the first successful run the pipeline stops eating reads and the app stays healthy on free tier. Side-effect: resources-article numbers now reflect the pool (~few thousand), not the old 205K count — smaller but honest. See [[gpj-firebase-credit-deadline]].
>
> **CAST-NET WIDENING + GSC HYGIENE (2026-09-24, founder: "I want all jobs"):** the harvester only searched ~50 broad buckets (8/day) so niche professional titles (e.g. "Account Retention") were never harvested and thus never searchable — a coverage gap, not just the pool-cap issue. Fixes (backend/static only, no `index.html`, no version bump): (1) **harvester** `US_ROLE_LIBRARY` expanded 50→**107** roles (account mgmt, customer success, recruiting, analysts, PM, healthcare/finance/legal specifics) + the daily **core** is now high-diversity **umbrella suffixes** ("manager"/"specialist"/"analyst"/"coordinator"/"representative"/"associate") that each surface a huge title spread EVERY run. Same role count per run → **daily writes stay flat** (< 20K). (2) **pool** national cap 3,000→**5,000** (env `POOL_NATIONAL_CAP`) so the new diversity reaches keyword search — costs NO extra build reads (still ~45), only a few more shard docs per client session. (3) **GSC:** `robots.txt` now `Disallow: /api/` (kills the `/api/apply-email` 4xx); the SEO generator's company-page prune now writes a **noindex + canonical + refresh-to-hub stub** instead of deleting, so rotated-out pages stop becoming 404s (the existing 38 historical 404s drop from Google naturally). Tests: benchmark GREEN · seo 21/0 · growth 16/0 (incl. pool) · resources 9/0 · harvester self-test. **Resources Article** self-heals now that quota + `_market_stats` are back (its 09-22 failure was the same 429 collateral). **Founder-side (skipped for now, DIY):** Gmail filter for DMARC report noise (mail is fine — passing via DKIM; SPF-fail is just ImprovMX forwarding); www→apex 301 in Vercel to clear the GSC "redirect"/"alternate" rows.
>
> **TIER B — FULL-CATALOG SEARCH INDEX (v279, 2026-09-24):** keyword search now reads a compact **search index covering ALL active jobs** instead of the ~5K deck pool, so it reaches every harvested title (works WITH Tier A: A fills the catalog with diverse titles, B makes them all findable — verified live: "retention" returns 0 on the deck pool but finds results via the index). **Dual-source (founder-requested):** PRIMARY = Firestore `job_pools/search-*` shards read only when searching (~a few reads/search); FALLBACK = a static `/search-index.json` on the CDN (**0 Firestore reads**) used automatically if the Firestore search read fails/caps. `fb.fetchSearchIndex()` (6h memory cache) → `searchAllJobsForKeyword` (falls back to the deck pool if the index is absent, so search never breaks). Index rows are **lite** (drop desc/req/benefits/matchTerms, same field names) → **medium** result cards (title/company/location/salary/ghost/apply); **match % appears on open** after the full posting lazy-loads (`_lite` cleared + recomputed). Deck stays small (fewer reads); search reaches everything. Build: `build_job_pool.mjs` emits the shards + the static file from the merged rows (no extra reads); harvest workflow commits `search-index.json` (git grows ~1MB/day — Firebase Storage is the no-bloat future option). Tests: `poolBuild.test.mjs` (searchIndexRows) + `[STATE-COVERAGE] v279` (end-to-end search-via-index + lite match suppression). **ACTIVATION:** the index populates on the next harvest/pool build; until then search gracefully falls back to the deck pool. **"FOR EMPLOYERS" ENTRY → LANDING + MENU ONLY (v280, 2026-09-25, founder-approved):** the "💼 For Employers →" link no longer clutters every screen. Removed the per-screen global-footer link + hid the per-screen nav tab; kept the **landing hero** link (guest entry) and added **"💼 For Employers" to the profile menu** (candidate entry; hidden in recruiter mode via `_gpjSyncEmployerNav`). Two prior tests updated to the new behaviour (v109 R9-A → landing/menu, v112 → day-counter core) + new `[STATE-COVERAGE] v280`. Full Playwright green.
>
> **v281 (2026-09-25, 3 founder fixes):** (1) **Match Preferences can be CLEARED** — a blank save now removes the field (+ persists the clear to the cloud, since cloudSync otherwise omits empties and the clear would return on reload); they were never required — the deck ranks by **location + résumé** (computeMatch: title/jobs/skills/edu/certs), prefs only *nudge* order (`gpjPrefBoost` = 0 when empty), and nothing hard-gates on them. (2) **Company logos** — added a last-resort name→domain guess (`_gpjGuessDomain`, e.g. "LGI Homes"→lgihomes.com) + a keyless **Clearbit** fallback in `_gpjLogoHtml`, so non-big-brand employers show a real logo; a wrong/dead guess 404s → honest emoji, and the **Website LINK stays real-only** (only the low-harm logo is guessed). Updated the v231+v240 logo tests to the new contract. (3) **Smart section transition** in the Browse job modal — `_gpjSummaryOnly` trims the Summary at the first Requirements/Benefits/Physical header + `_gpjDedupText`, so it ends cleanly on overview/duties/values instead of bleeding the benefits tail above a duplicate Benefits section (fail-safe: only trims when a header is found and ≥120 chars remain). +[STATE-COVERAGE] v281. **NEXT — NOTIFICATIONS (approved approach, not yet built):** login "New Job Matches" is only a transient toast (`maybeAlertNewMatches`) — never added to the bell; none of the 3 toggles email yet. Plan: (a) add a persistent BELL notification for fresh high matches on login; (b) restructure each toggle to main on/off + a nested "Also email me" sub-toggle (email only if both on); (c) wire emails via the **batched, DRY-RUN-gated weekly candidate digest** (Resend, [FREE-TIER] — never per-event), honoring the sub-toggles; (d) **Ghost Risk** → in-app dismissible heads-up when opening a high-risk company + email ONLY about companies the user ALREADY applied to that have high ghost-risk / a new ghost report (actionable, not noise). **LEVEL-UP referral** planned in `docs/LEVELUP-REFERRAL-PLAN.md` (affiliate networks + templates + HELD disclosure copy); reminder scheduled 2026-09-29 to get tracking links before wiring.
>
> **NOTIFICATIONS v282 (2026-09-25, in-app — SHIPPED):** (1) **New Job Matches now hits the 🔔 bell on login** — `maybeAlertNewMatches` adds a persistent, once/day bell notification (was toast-only, so the bell showed nothing); persisted via `gpj_nm_notif` + re-added in `_notifCandidate` so it survives list rebuilds; gated on the toggle + seen-set. (2) **Nested "Also email me" sub-toggle** under each of the 3 alert toggles — shown only when the main is ON, **DEFAULT OFF (opt-in)**, saved as `preferences[key+'Email']` for the v283 sender; `_gpjSyncNotifEmailRow` handles show/hide + restore; `notifEmailOn`/`setNotifEmailPref` added. **Nothing emails until BOTH the main + the email sub-toggle are on** (founder requirement). (3) Reworded the 3 toggles honestly (in-app now; email opt-in below; Ghost Risk email = companies you *applied to*). +[STATE-COVERAGE] v282. **v283 NEXT (email wiring + ghost dismissible):** extend the **batched, DRY-RUN-gated weekly candidate digest** to send new-matches / applied-to-ghost-risk / rate-reminders, each gated by its sub-toggle + honoring `emailOptOut`; upgrade the Ghost-Risk in-app heads-up to a **dismissible notice**. See memory [[gpj-search-index-test-live-data]] for the tests-must-mock-fetchSearchIndex gotcha.
>
> **NOTIFICATIONS EMAIL WIRING — "v283" (2026-09-25, backend-only — no app-version bump; the app half shipped in v282):** the weekly candidate digest (`scripts/candidate_digest.mjs`) is now the OTHER half of the v282 split and sends ONE batched email per candidate from up to three sections, **each gated by its own `*Email` sub-toggle (DEFAULT OFF)** — so the in-app bell toggles never trigger email: (A) **new job matches** (shared card scorer, unchanged), (B) a **ghost-risk heads-up on companies they APPLIED to** that the community flagged (honest report COUNT, never a fabricated %; threshold `DIGEST_GHOST_MIN`=3), (C) a **nudge to rate a company** they applied to but haven't rated. The critical re-gate: eligibility moved from `newJobMatches !== false` (the in-app pref) to the **`*Email` opt-in** — otherwise going live would have emailed everyone with the bell on. Honors the **canonical opt-out** (`email_suppress` + `emailUnsub`/`preferences.emailUnsub`) and appends the shared **CAN-SPAM footer** (real contact + one-click unsubscribe). **[FREE-TIER]:** sections B/C need ZERO extra per-candidate reads (applied list + `vibeReviews` are already on the profile doc); +1 shared bounded `ghost_reports` read + 1 shared bounded `email_suppress` read. **Still DRY-RUN gated:** workflow cron COMMENTED + `DIGEST_LIVE` unset — founder runs `workflow_dispatch`, reviews the preview, then flips it live. Tests: `tests/email/candidateDigest.test.mjs` rewritten to the opt-in contract (email suite 29/0; backend 146/0). **STILL PENDING (needs `[UI-REVIEW]` before code): the in-app dismissible Ghost-Risk NOTICE** (approach approved in concept — proposing the specific visual before building, per the UI-REVIEW gate).
>
> **Current live build: v282** (`CACHE_VERSION = gpj-v282`; verified 2026-09-25). **v278 founder-repro UI fixes:** (1) the **blank req-gap pill** on the card face — CSS rule `.job-card .risk-gap-row>span{display:flex!important}` forced the empty `.s-req` visible past `_paintReqPill`'s inline `display:none`; added a `.s-req:empty{display:none!important}` guard (mirrors the ghost chip) + clear its text in the hide branch, so the pill is hidden until it carries a real "⚠️ N gaps" / "✓ No gaps" label. (2) the **notification bell** rode high in its pill (centered only by `line-height`) → flex-centered (`align/justify:center`, inline so it holds at any width; `_gpjNotifPaint` sets `display:flex` when signed in). (3) **Job Expectations truncation** (text cut mid-word at the top when expanded) — **DEFERRED, pending founder decision.** The flow-fully fix (`max-height:none;overflow:visible`) directly conflicts with the **v83 scroll-region behaviour** (the expanded box is an intentional 360px internal scroll that doesn't collapse on inner taps; a v83 [STATE-COVERAGE] test enforces it). Reverted before commit rather than silently rewrite a prior founder-approved behaviour. Options to resolve: (A) flow fully (supersede v83, update its test), (B) enlarge the scroll region to ~70vh so normal postings don't clip but very long ones still scroll, or (C) keep v83 as-is + ensure the box always opens at scroll-top. Awaiting choice. Also verified in this session: the **Cloudflare "domain removed" email is benign** (expected result of keeping DNS on Vercel; the AI Worker on `…workers.dev` returned HTTP 200, unaffected); **GSC** "Duplicate without user-selected canonical" now **Passed**, the "discovered/crawled – not indexed" rows are the parked templated pages (crawl reality, not a bug). Sprint v270→v276, all **live-verified**. v270 C4 desktop drag/header/rail-leak · v271 footer/logo/candidate-toast/ghost-% chip · v272 **N24** Applicants clickable counters · v273 **N28** mobile recruiter pipeline · v274 notification-toggle honesty (default ON to match in-app behaviour) · v275 **P2-6 My Data** (real transparency view + real JSON export replacing the fake, live-verified with real data) + phone optional at signup · v276 **Role Fit unified to the card engine** (median of computeMatch across a live-posting sample — Role Fit went 100→72 to match the ~77% cards, live-verified, computeMatch called 40×). **N8 post-deploy smoke shipped.** N27 (reverse-match) verified running. **Full candidate-side + AI live-tested in the founder's session** (deck/ghost-chip/rater/Match-to-Job/Cover-Letter/Jett — all work + honest, zero fabrication). Read-cost is a non-issue (~$0.09/day; founder to set a $5 Firebase cap). **v277** (gating): (A) req-pill honesty — the swipe card no longer shows "✓ No gaps" on a clipped/unhydrated posting (only after the full text is seen); (B) SEO — JSON-LD structured data on the homepage + résumé checker; (C) SEO — 3 cornerstone evergreen "ghost job" article templates added to the Resources engine. **P1-3 candidate email digest SHIPPED (dry-run gated)** — makes the "email later" toggles real. **GTM one-pager PDF** for sharing with users: `docs/GhostProofJob-Overview.pdf` (regen: `python scripts/build_gtm_pdf.py`). **SEO status: ~6/10 — strong technical foundations + live content engine, but early (needs backlinks + content volume + time). #1 lever = backlinks (get the free checker + "ghost job" angle into communities).** **GROWTH WAVE 1 shipped (2026-09-02, no app-version bump — checker + resources engine only, index.html byte-unchanged):** (1) **Checker share loop** — `resume-checker.html` now offers, after a score, a **Download score card** (branded PNG rendered client-side via `<canvas>` — score + brand only, NEVER résumé text/PII), **Copy link** (`?ref=share` backlink), and a native **Share** button on mobile (`navigator.share`). 100% static, 0 functions, 0 reads, nothing stored. Live-verified desktop+mobile; +1 [STATE-COVERAGE] test (all 4 quadrants). (2) **Cornerstone wave 2** — 5 more evergreen high-intent article templates in `build_resources.mjs` (`ghost-interview`, `ghost-follow-up`, `resume-ats`, `cover-letter-real`, `find-responsive-employers`) — enter the rotation automatically, publish one-per-run. All 9 resources tests green.
...
- [ ] **GSC "Page with redirect" / www duplicate (low priority, SEO nit):** `www.ghostproofjob.com` currently serves a 200 instead of redirecting to the apex. The canonical tag already covers it, so it's not a real problem — but for a clean signal, in **Vercel → Project → Settings → Domains**, set `www.ghostproofjob.com` to **Redirect to `ghostproofjob.com`** (308). No code change. The "discovered/crawled – currently not indexed" rows are the parked templated pages (crawl reality), not a fix.
