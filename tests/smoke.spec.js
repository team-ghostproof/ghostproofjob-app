...
/* ─── [STATE-COVERAGE] network-state helpers (CLAUDE.md §3 rule 9) ───────────
   Quadrant 3 (Interrupted/Failed network) and Quadrant 4 (Empty/Missing data)
   for any surface that talks to Firestore or the Cloudflare Worker.

   Usage in a spec (BEFORE the page.goto / the action that fetches):
   ...
   NOTE: Firestore reads go over a WebChannel protocol, so for Firestore the
   high-fidelity "empty data" simulation is usually mockNetworkFailure (the
   app's catch/degrade paths) or seeding via page.evaluate — a plain '[]' body
   only fully applies to REST-shaped endpoints like the Worker. Pass a custom
   body when an endpoint expects a specific empty shape.                      */
