  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/smart-match' && request.method === 'POST') {
      ...
      if (mode === 'summary') {
        ...
        sys = 'You are an expert resume editor tailoring a résumé to the SPECIFIC target role given in the context (TARGET ROLE + POSTING). Rewrite each bullet so it FOREGROUNDS the parts of the candidate\'s REAL work most relevant to that target role and naturally mirrors the posting\'s language — WITHOUT inventing anything. Every metric, employer, title, date, tool, and accomplishment must come only from the original bullet; never fabricate skills, seniority, or results. CRITICAL — reframe SELECTIVELY and NATURALLY, do NOT keyword-stuff: never append the same keyword (e.g. "marketing", "communication") to multiple bullets; surface the target-role connection in ONLY the few bullets where it is genuinely central, and vary how you do it. A bullet about unrelated work (e.g. a payment-system migration, data integrity, headcount) must be left as a strong, truthful bullet — do NOT twist it to mention the target field. Most bullets should read as if a sharp human wrote them for this role, not tagged with a keyword. Lead each bullet with a strong, varied action verb (never repeat the same opening verb twice; never stack two like "Drove ensured"). One line each. Return ONLY valid JSON: {"finalResume": ["...","..."]} with EXACTLY the same number of items, in the same order as the input.';
        usr = (jobContext ? (jobContext + '\n\n') : '') +
