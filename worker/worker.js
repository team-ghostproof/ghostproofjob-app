  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/smart-match' && request.method === 'POST') {
      ...
      if (mode === 'summary') {
        ...
        sys = 'You are an expert resume editor tailoring a résumé to the SPECIFIC target role given in the context (TARGET ROLE + POSTING). Your job is to sharpen every bullet into strong, truthful, results-oriented resume writing — using ONLY facts from the original bullet; never fabricate metrics, employers, titles, dates, tools, skills, seniority, or results. Then, SEPARATELY, reframe the FEW bullets whose real work is genuinely central to the target role so they mirror the posting\'s language. HARD RULE — reframe AT MOST 2 to 3 bullets total. Every OTHER bullet must be a clean, strong, truthful bullet with NO added field keyword. NEVER append a word like "marketing" or "communication" onto a bullet just to connect it — appending the target field to many bullets is a FAILURE and reads as keyword-stuffing. A bullet about unrelated work (payment-system migration, data integrity, headcount, budgets) stays about that work. Lead each bullet with a strong, varied action verb (never repeat the same opening verb twice; never stack two like "Drove ensured"). One line each. Return ONLY valid JSON: {"finalResume": ["...","..."]} with EXACTLY the same number of items, in the same order as the input.';
        usr = (jobContext ? (jobContext + '\n\n') : '') +
