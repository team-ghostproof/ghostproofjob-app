...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/smart-match' && request.method === 'POST') {
      ...
      let sys, usr;
      if (mode === 'summary') {
        sys = 'You are an expert resume writer composing a PROFESSIONAL SUMMARY. Using ONLY facts present in the provided text and any FACTS block, write ONE polished professional summary of 2 to 3 sentences (roughly 45-80 words). Synthesize years of experience, domains, and real scope numbers where they appear. Never invent metrics, employers, titles, dates, or experience. No bullet points, no first-person "I", no filler like "results-driven", "proven track record", or "dynamic professional". Lead with the strongest concrete capability. Return ONLY valid JSON: {"finalResume": ["<the summary as a single string>"]}.';
        usr = (jobContext ? (jobContext + '\n\n') : '') +
              (keywords.length ? ('Target keywords: ' + keywords.join(', ') + '\n\n') : '') +
              'Current summary / source text to elevate:\n' + bullets.join('\n');
      } else {
        sys = 'You are an expert resume editor. Rewrite each resume bullet to be sharper, results-oriented, and naturally aligned to the target keywords — using ONLY facts implied by the original bullet. Never invent metrics, employers, titles, dates, or experience. Keep each bullet to one line, lead with a strong action verb (never repeat the same opening verb twice, and never stack two verbs like "Drove ensured"), and preserve the candidate’s real meaning. Return ONLY valid JSON: {"finalResume": ["...","..."]} with EXACTLY the same number of items, in the same order as the input.';
        usr = (jobContext ? (jobContext + '\n\n') : '') +
